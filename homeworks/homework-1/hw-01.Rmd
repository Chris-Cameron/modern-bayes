...
```
I used the \texttt{read.table} command to convert the dataset into a data frame.
...
I would use the \texttt{colnames} command to get the column names in rain.df
...
I would use the \texttt{rain.df[4,2]} command to get the value, which is \texttt{4}
...
I would use the \texttt{rain.df[2,]} command to get the value, the content is below:
...
I would use the \texttt{ggplot(rain.df, aes(x = daily)) + geom_histogram()}
...
I would use the \texttt{rain.df[rain.df < 0] <- NA} command to fix the data
...
The purpose of reproducible code is for others to be able to readily use it
when collaborating.

b.) Given an example of why making your code reproducible is important for you to know in this class and moving forward. 

If code is not reproducible, it is much more difficult to analyze how someone got a result
or help someone resolve an issue they are having.
