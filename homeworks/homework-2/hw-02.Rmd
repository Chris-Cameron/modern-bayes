...
galenshore <- function(th,c,d){
  e <- exp(1)
  y <- (2*((d^(2*c))*(th^(2*c-1))*e^((-d^2)*(th^2))))/gamma(c)
  return(y)
}
...
$$ E[\theta \mid y_{1:n}] = \int\theta*\theta^{2(an+c)-1}e^{-\theta(d^2 +\sum_{i = 1}^{n} y_i^2)} d\theta$$

$$ E[\theta \mid y_{1:n}] = \int\theta^{2(an+c)}e^{-\theta(d^2 +\sum_{i = 1}^{n} y_i^2)} d\theta$$
...
$$=p(y \mid \theta) = \frac{2}{\Gamma(a)} \; \theta^{2a} y_{n+1}^{2a - 1} e^{-\theta^2 y_{n+1}^2} * $$